...
@pytest.mark.parametrize(
    'secret, should_flag',
    [
        ('sk_live_ReTllpYQYfIZu2Jnf2lAPFjD', True),
        ('rk_live_5TcWfjKmJgpql9hjpRnwRXbT', True),
        ('pk_live_j5krY8XTgIcDaHDb3YrsAfCl', False),
        ('sk_live_', False),
    ],
)
def test_stripeaccesskey_detector_detects_expected_secrets(run_with_detector, secret: str, should_flag: bool):
    """
    Integration-style test against the StripeDetector.

    Uses the same payload set as detect-secrets itself to ensure we
    correctly wire the StripeDetector via our inspection pipeline.
    """
    # Arrange
    ...
    finding: Finding | None = _get_finding(resp.findings, "secret_stripe_access_key")
