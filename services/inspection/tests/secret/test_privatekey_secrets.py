...
def test_privatekey_detector_detects_expected_secrets(run_with_detector, secret: str, should_flag: bool):
    """
    Integration-style test against the PrivateKeyDetector.

    Uses the same payload set as detect-secrets itself to ensure we
    correctly wire the PrivateKeyDetector via our inspection pipeline.
    """
    # Arrange
    ...
    finding: Finding | None = _get_finding(resp.findings, "secret_private_key")
