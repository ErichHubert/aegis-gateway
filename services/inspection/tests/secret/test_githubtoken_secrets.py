...
def test_githubtoken_detector_detects_expected_secrets(run_with_detector, secret: str, should_flag: bool):
    """
    Integration-style test against the GitHubTokenDetector.

    Uses the same payload set as detect-secrets itself to ensure we
    correctly wire the GitHubTokenDetector via our inspection pipeline.
    """
    # Arrange
    ...
    finding: Finding | None = _get_finding(resp.findings, "secret_github_token")
