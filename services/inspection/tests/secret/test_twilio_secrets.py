...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx', True),
    ],
)
def test_twilioapikey_detector_detects_expected_secrets(run_with_detector, secret: str, should_flag: bool):
    """
    Integration-style test against the TwilioKeyDetector.

    Uses the same payload set as detect-secrets itself to ensure we
    correctly wire the TwilioKeyDetector via our inspection pipeline.
    """
    # Arrange
    ...
    finding: Finding | None = _get_finding(resp.findings, "secret_twilio_api_key")
