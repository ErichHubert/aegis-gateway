...
from core.detectors.protocols import IPiiDetector


class PiiDetectionOrchestrator(IPiiDetector):
    """High-level PII detector orchestrating multiple lower-level detectors."""

    def __init__(self, detectors: Sequence[IPiiDetector]) -> None:
        if not detectors:
            raise ValueError("PiiDetectionOrchestrator requires at least one detector")
        self._detectors: list[IPiiDetector] = list(detectors)
